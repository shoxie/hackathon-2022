      ...
      <div className="grid grid-cols-4">
        {plans.map((plan) => (
          <div key={plan.id} className="text-center">
            ...
            <div className="pt-5">
              <span className="text-2xl font-medium text-gray-300"><span className="text-secondary">{plan.places.length}</span> địa điểm</span>
            </div>
            <div className="flex items-center justify-center pt-5">
          <Link href={`/plans/${plan.id}`} as={`/plans/${plan.id}`}>
              <a className="px-8 py-2 font-semibold text-white rounded-xl bg-secondary">Xem chi tiết</a>
          </Link>
      </div>
          </div>
        ))}
